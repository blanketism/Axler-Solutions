...
\begin{sol}
    This is a generalization of \autoref{ex:2a2} and a direct consequence of the fact that $v_k = w_k - w_{k-1}$ for $k \in \set{2, \ldots, m}$ and $v_1 = w_1$.
...
\begin{sol}
    ...
    Then $v_1, v_2$ and $w_1, w_2$ are linearly independent lists of vectors in $\r^2$.
	However,
...
\begin{sol}
	...
	Let $\beta_1, \ldots, \beta_m \in \f$ such that
    \[\beta_1 v_1 + \cdots + \beta_m v_m = 0.\]
...
\begin{sol}
    Since the list $1, x, x^2, x^3, x^4$ is a linearly independent list in $\pp_4(\f)$ and has length 5, and the length of a spanning list cannot be less than the length of a linearly independent list, no list of four polynomials can span $\pp_4(\f)$.
\end{sol}
...
\begin{sol}
	...
	Then by \autoref{ex:2a13}, the list $v_1, \ldots, v_m, v_{m+1}$ is linearly independent.
...
\begin{sol}
    By \autoref{ex:2a17}, it suffices to construct a sequence of vectors $v_1, v_2, \ldots$ in $\f^\infty$ such that $v_1, \ldots, v_m$ is linearly independent for every positive integer $m$.
	Let $v_k$ be the vector in $\f^\infty$ whose $k$-th coordinate is $1$ and all other coordinates are $0$.
	Explicitly, we have
	...
	Thus, by \autoref{ex:2a17}, $\f^\infty$ is infinite-dimensional.
\end{sol}
...
\begin{sol}
    By \autoref{ex:2a17}, it suffices to construct a sequence of continuous real-valued functions $f_1, f_2, \ldots$ on the interval $[0, 1]$ such that $f_1, \ldots, f_m$ is linearly independent for every positive integer $m$.
	Let $f_k(x) = x^{k-1}$ for each positive integer $k$.
	Then for any positive integer $m$, let $\alpha_1, \ldots, \alpha_m \in \r$ such that
	...
	Therefore, by \autoref{ex:2a17}, the real vector space of all continuous real-valued functions on the interval $[0, 1]$ is infinite-dimensional.
\end{sol}
 
\begin{exercise}[2a20]
    Suppose $p_0, p_1, \ldots, p_m$ are polynomials in $\pp_m(\f)$ such that $p_k(2) = 0$ for each $k \in \set{0, \ldots, m}$.
	Prove that $p_0, p_1, \ldots, p_m$ is not linearly independent in $\pp_m(\f)$.
\end{exercise}

\begin{sol}
    Since $p_k(2) = 0$ for each $k \in \set{0, \ldots, m}$, we have
    \[p_k(x) = (x - 2)q_k(x)\]
    for some polynomial $q_k(x) \in \pp_{m-1}(\f)$.
	Then we may write
    \[p_0(x), p_1(x), \ldots, p_m(x) = (x - 2)q_0(x), (x - 2)q_1(x), \ldots, (x - 2)q_m(x).\]
    Since $\pp_{m - 1}(\f)$ can be spanned by the $m$ polynomials $1, x, \ldots, x^{m-1}$, the list of $m + 1$ polynomials $q_0, q_1, \ldots, q_m$ must be linearly dependent.
