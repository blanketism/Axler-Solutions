...
\begin{sol}
    Let $U = \set{(x, y, z) \in \fbb^3 \mid x + y + z = 0}$.
    Rewriting $z = -x - y$, we have
    \[
        U = \set{(x, y, -x - y) \mid x, y \in \fbb}.
    \]
    Consider the list of vectors
    \[
        v_1 = (1, 0, -1), \quad v_2 = (0, 1, -1), \quad v_3 = (2, -1, -1), \quad v_4 = (1, 1, -2).
    \]
    For any $(z, w, -z - w) \in U$, we may write it as $z v_1 + w v_2$.
    Therefore, $v_1, v_2, v_3, v_4$ span $U$.
\end{sol}
...
\begin{sol}
    If $w = 0$, then $w \in \spn(v_1, \ldots, v_m)$ and we are done.
    Otherwise, assume $w \neq 0$, and let $\alpha_1, \ldots, \alpha_m \in \fbb$, not all zero, such that
    \[
...
\begin{soliff}
    \rightimp We proceed by contrapositive.
    Suppose $w \in \spn(v_1, \ldots, v_m)$, but $v_1, \ldots, v_m, w$ is linearly independent.
    Then $w$ is a linear combination of $v_1, \ldots, v_m$, contradicting the linear independence of $v_1, \ldots, v_m, w$.
    Hence, $w \notin \spn(v_1, \ldots, v_m)$.

    \leftimp Conversely, suppose $w \notin \spn(v_1, \ldots, v_m)$, but $v_1, \ldots, v_m, w$ is linearly dependent.
    By the linear dependence lemma, there exists $k \in \set{1, \ldots, m}$ such that $v_k \in \spn(v_1, \ldots, v_{k - 1})$.
    This contradicts the linear independence of $v_1, \ldots, v_m$.
    Hence, $v_1, \ldots, v_m, w$ must be linearly independent.
\end{soliff}
...
\begin{sol}
	...
	Since there is no maximum length for a linearly independent list of vectors in $V$, no list can span $V$.
    Therefore, $V$ is infinite-dimensional.
\end{sol}
...
\begin{sol}
    Note that $\pcal_m(\fbb)$ is spanned by the list $1, z, \ldots, z^m$ of $m + 1$ vectors, so there cannot be a list of $m + 2$ linearly independent vectors in $\pcal_m(\fbb)$. 
    Suppose $p_0, \ldots, p_m$ is linearly independent in $\pcal_m(\fbb)$.
    Moreover, notice that every linear combination of $p_0, \ldots, p_m$ is a polynomial such that
    \[
        \sum_{i = 0}^m \alpha_i p_i(2) = 0.
    \]
    Since $q = 1 \in \pcal_m(\fbb)$ is a polynomial where $q(2) \neq 0$, $q \notin \spn(p_0, \ldots, p_n)$.
    By \exref{2a13}, the list $p_0, \ldots, p_m, q$ is of length $m + 2$ that is linearly independent, which is a contradiction.
    Hence, $p_0, \ldots, p_m$ cannot be linearly independent in $\pcal_m(\fbb)$.
\end{sol}