...
\begin{sol}
    Let $V$ be a vector space with exactly one basis, call it $v_1, \dots, v_n$.
    Consider that for $v \in V$, the list containing only $v$ is linearly independent, so we may extend the list $v$ to a basis for $V$.
    This means that $v = v_k$ for some $k \in \{1, \dots, n\}$, so every vector in $V$ is one of $v_1, \dots, v_n$.
    For nonzero $\lambda \in \fbb$ and $k \in \{1, \dots, n\}$, consider $\lambda v_k \in V$.
    If $\lambda v_k = v_j$ for some $j \neq k$, then $v_1, \dots, v_n$ is not linearly independent, a contradiction.
    Therefore, $\lambda v_k = v_k$, and thus $(\lambda - 1)v_k = 0$.
    Because $\lambda$ was arbitrary, it must be that $v_k = 0$.
    Because $k$ was arbitrary, we have $v_k = 0$ for all $k \in \{1, \dots, n\}$, so the basis for $V$ is empty.
    $\spn(\,) = \{0\}$, so the trivial vector space $V = \{0\}$ is the only vector space with exactly one basis.
\end{sol}
...
\begin{exercise}{2b3}
    \begin{subexercises}
        ...
        \item Find a subspace $W$ of $\rbb^5$ such that $\rbb^5 = U \oplus W$.
    \end{subexercises}
\end{exercise}
...
\begin{exercise}{2b4}
    \begin{subexercises}
        ...
        \item Find a subspace $W$ of $\cbb^5$ such that $\cbb^5 = U \oplus W$.
    \end{subexercises}
\end{exercise}
...
\begin{sol}
	...
	Because every spanning list of $V$ can be reduced to a basis of $V$, $u_1, \ldots, u_m, w_1, \ldots, w_n$ can be reduced to a basis of $V$ consisting of vectors in $U \cup W$.
\end{sol}
...
\begin{sol}
    Let $v \in V$.
    Since $V = U \oplus W$, there are unique $u \in U$ and $w \in W$ such that $v = u + w$.
    Since $u_1, \ldots, u_m$ spans $U$ and $w_1, \ldots, w_n$ spans $W$, there exists scalars $\alpha_1, \ldots, \alpha_m$ and $\beta_1, \ldots, \beta_n$ such that
    \[
        u = \sum_{i=1}^m \alpha_i u_i \quad \text{and} \quad w = \sum_{j=1}^n \beta_j w_j.
    \]
    Therefore,
    \[
        v = u + w = \sum_{i=1}^m \alpha_i u_i + \sum_{j=1}^n \beta_j w_j,
    \]
    which shows that the list spans $V$.
    To show linear independence, suppose $v = 0$ so that $v = u + w$ imply $u = w = 0$.
    Since $u_1, \ldots, u_m$ and $w_1, \ldots, w_n$ are linearly independent, it follows that $\alpha_1 = \cdots = \alpha_m = 0$ and $\beta_1 = \cdots = \beta_n = 0$.
    Therefore, the list $u_1, \ldots, u_m, w_1, \ldots, w_n$ is linearly independent.
    Combining this with the fact that it spans $V$, we conclude that it is a basis of $V$.
\end{sol}
