...
\begin{sol}
    Suppose $T$ is invertible. Then there exists $S \in \lin{W, V}$ such that $ST = I_V$ and $TS = I_W$. Taking the dual of both sides, we get $T'S' = I_{W'}$ and $S'T' = I_{V'}$, which shows that $T'$ is invertible with inverse $S'$.

    Conversely, suppose $T'$ is invertible. Then $T'$ is an isomorphism. Then
    \[\dim V = \dim V' = \dim W' = \dim W.\]
    By injectivity of $T'$, we have $\nul T' = \set 0$. Since $\nul T' = (\range T)^0$, we have $(\range T)^0 = \set 0$ so that $\range T = W$. Hence, $T$ is surjective. Since $\dim V = \dim W$, we conclude that $T$ is injective as well. Therefore, $T$ is invertible.
\end{sol}
...
\begin{sole}
    \item Since $(\operatorname{range} T)^0=\operatorname{null} T'$ and $\operatorname{null} T'=\operatorname{span}(\varphi)$, we have $(\operatorname{range} T)^0=\operatorname{span}(\varphi)$. Since $(U^0)^0=U$ in finite dimensions, taking annihilators yields
