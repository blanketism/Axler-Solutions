...
\begin{sol}
    ...
    \begin{cases}
        ...
        0 & \text{if } k \neq i.
    \end{cases}\]
    In other words, $S_{ij}$ sends $v_i$ to $v_j$ and sends all other basis vectors to $0$. Write $Tv_i = \alpha_{i1} v_1 + \cdots + \alpha_{in} v_n$. Then
    \[S_{ij}Tv_i = S_{ij}(\alpha_{i1} v_1 + \cdots + \alpha_{in} v_n) = \alpha_{ii} v_j,\]
    and
    \[TS_{ij}v_i = Tv_j = \alpha_{j1} v_1 + \cdots + \alpha_{jn} v_n.\]
    Since $j$ was arbitrary, we have $Tv_k = \alpha_{ii}v_k$ for all $k$, and since $i$ was arbitrary, we have $\alpha_{11} = \alpha_{22} = \cdots = \alpha_{nn} = \lambda$, hence $T = \lambda I$.
\end{sol}
