...
\begin{sol}
    ...
    \[T(2, 2, 2) = (2, 12 + 8c) = (2, 12 + c), = 2T(1, 1, 1),\]
...
\begin{sol}
    ...
    The integral terms may be subtracted, so we get $c \sin(2p(0)) = 2c \sin(p(0))$ for all $p \in \pp(\r)$. Consider $p(x) = \pi/2$. Then $\sin(2p(0)) = \sin(\pi) = 0$ and $2\sin(p(0)) = 2\sin(\pi/2) = 2$. Therefore, $c = 0$.
...
\begin{sol}
    Let $e_1, \ldots, e_n$ be the standard basis of $\f^n$. Then for each $k = 1, \ldots, n$, we have
...
\begin{sol}
    We must first ensure that the operations of addition and scalar multiplication are well-defined, i.e., if $S, T \in \lin{V, W}$ and $\alpha \in \f$, then $S + T \in \lin{V, W}$ and $\alpha T \in \lin{V, W}$. We first show that $S + T$ is linear. Let $v, u \in V$. Then
    \begin{align*}
        (S + T)(v + u) & = S(v + u) + T(v + u) \\
        & = (Sv + Su) + (Tv + Tu) \\
        & = (Sv + Tv) + (Su + Tu) \\
    ...
    \begin{align*}
        ...
        & = \alpha (S + T)v.
    \end{align*}
    This shows that $S + T$ is linear. Now we show that $\alpha T$ is linear. Let $v, u \in V$. Then
    \begin{align*}
...
\begin{exercise}[3a7]
    Show that every linear map from a one-dimensional vector space to itself is multiplication by some scalar. More precisely, prove that if $\dim V = 1$ and $T \in \lin{V}$, then there exists $\lambda \in \f$ such that $Tv = \lambda v$ for all $v \in V$.
\end{exercise}
...
\begin{exercise}[3a8]
    ...
    \remark{This was a particularly difficult exercise because all obvious examples of functions that are homogeneous are also additive. The key idea is to utilize composition of functions.}
\end{exercise}
...
\begin{sol}
    ...
    Conversely, suppose $ST = TS$ for every $S \in \lin{V}$. We claim that $v$ and $Tv$ are linearly dependent for every $v \in V$. Suppose not. Then there exists $v \in V$ such that $v$ and $Tv$ are linearly independent. Extend $\set{v, Tv}$ to a basis of $V$. Define $S \in \lin{V}$ by sending $v$ to itself and sending $Tv, v_3, \ldots, v_n$ to $0$. Then
    ...
    Now let $v \in V$ be nonzero. Then there exists $\lambda \in \f$ such that $Tv = \lambda_v v$. We claim that $\lambda_v$ is independent of the choice of $v$. Let $v_1, \ldots, v_n$ be a basis of $V$.
    \begin{itemize}
        \item If $n = 1$, then the claim is immediate, since $Tv_1 = \lambda_{v_1} v_1$.
        \item If $n \geq 2$, let $v_i, v_j$ be two distinct basis vectors. Then their linear independence implies
        \[\lambda_{v_i + v_j} v_i + \lambda_{v_i + v_j} v_j = T(v_i + v_j) = Tv_i + Tv_j = \lambda_{v_i} v_i + \lambda_{v_j} v_j.\]
        Since $v_i$ and $v_j$ are linearly independent, it follows that $\lambda_{v_i + v_j} = \lambda_{v_i} = \lambda_{v_j}$. Therefore, $\lambda_v$ is independent of the choice of $v$, and we may denote it by $\lambda$.
    \end{itemize}
...
\begin{sol}
    ...
    Fix some nonzero $a_i$ among $a_1, \ldots, a_m$. Let $w_1, \ldots, w_m \in W$ be defined by
    \[w_k =
    \begin{cases}
        0 & \text{if } k \neq i, \\
        w & \text{if } k = i,
...
\begin{sol}
    ...
    Hence, $E^* = I \in \ee$. Since $\ee$ is a two-sided ideal, $I = E^* \in \ee$ implies that for any $T \in \lin V$, we have $TI = T \in \ee$. Therefore, $\ee = \lin V$.
\end{sol}