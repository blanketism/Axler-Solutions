...
\begin{sol}
    Since $\dim V / U = \dim \f = 1$, then there exists some isomorphism $\psi \in \lin{V /U, \f}$. Let $\pi: V \to V / U$ be the quotient map. Define $\varphi = \psi \circ \pi \in \lin{V, \f}$. Suppose $\varphi(v) = 0$. Then $\psi(\pi(v)) = 0$, which implies $\pi(v) = 0$ since $\psi$ is an isomorphism. Hence, $v \in U$, and we have $\nul \varphi \subseteq U$. Conversely, if $v \in U$, then $\pi(v) = 0$, so $\varphi(v) = \psi(\pi(v)) = 0$. Therefore, $\nul \varphi = U$.
\end{sol}
