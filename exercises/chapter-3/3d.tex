...
\begin{sol}
    ...
    \[AE_{j, k} = E_{j, k}A\]
    implies that the $j$-th column of $A$ is the same as the $k$-th column of $A$. Since this is true for all $j, k = 1, \ldots, n$, then all columns of $A$ are the same. Let $\lambda$ be the entry in the first column of $A$. Then $A = \lambda I$, and thus $T$ is a scalar multiple of the identity operator.
...
\begin{sol}
    ...
    \begin{itemize}
        \item If $\deg p \geq 2$, it is clear that $(x^2 + x) p''$ has degree at least $2$, and thus the left-hand side has degree at least $2$. However, the right-hand side is a constant, which is a contradiction.
...
\begin{sol}
    Let $T : \f^{n, 1} \to \f^{n, 1}$ be defined by $Tx = Ax$ for $x \in \f^{n, 1}$. Since $AB = I$, then for any $x \in \f^{n, 1}$, we have
    \[TBx = A(Bx) = (AB)x = Ix = x.\]
    Hence, $T$ is invertible and $T\inv = B$. Therefore,
    \[BAx = B(Tx) = T\inv (Tx) = x\]
