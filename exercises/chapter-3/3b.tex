...
\begin{sol}
    Let $T \in \lin{V}$ be such that $\nul T$ and $\range T$ are both finite-dimensional. Let $u_1, \ldots, u_m$ be a basis of $\nul T$, and let $w_1, \ldots, w_n$ be a basis of $\range T$. Let $v_j \in V$ be such that $Tv_j = w_j$ for $j = 1, \ldots, n$. We claim that the list $u_1, \ldots, u_m, v_1, \ldots, v_n$ is a basis of $V$.

    To show that this list spans $V$, let $v \in V$. Then $Tv \in \range T$, so we may write
    ...
    To show that the list is linearly independent, suppose
    \[\gamma_1 u_1 + \cdots + \gamma_m u_m + \delta_1 v_1 + \cdots + \delta_n v_n = 0\]
    for some scalars $\gamma_1, \ldots, \gamma_m, \delta_1, \ldots, \delta_n \in \f$. Applying $T$ to both sides, we have $T(\gamma_1 u_1 + \cdots + \gamma_m u_m) + T(\delta_1 v_1 + \cdots + \delta_n v_n) = 0$. Since $u_1, \ldots, u_m \in \nul T$, we have $T(\gamma_1 u_1 + \cdots + \gamma_m u_m) = 0$. Therefore,
    \[T(\delta_1 v_1 + \cdots + \delta_n v_n) = \delta_1 Tv_1 + \cdots + \delta_n Tv_n = \delta_1 w_1 + \cdots + \delta_n w_n = 0.\]
    Since $w_1, \ldots, w_n$ is a basis of $\range T$, it follows that $\delta_1 = \cdots = \delta_n = 0$. Thus, we have
    \[\gamma_1 u_1 + \cdots + \gamma_m u_m = 0.\]
    Since $u_1, \ldots, u_m$ is a basis of $\nul T$, it follows that $\gamma_1 = \cdots = \gamma_m = 0$. Therefore, the list $u_1, \ldots, u_m, v_1, \ldots, v_n$ is linearly independent.
\end{sol}
