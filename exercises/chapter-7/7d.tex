...
\begin{soliff}
    ...
    \leftimp Let $u \in V$ with $\norm{u} = 1$.
    By 6.36 and $\dim V \geq 2$, extend $u$ to an orthonormal list $u, v$ of length two in $V$.
    By hypothesis, $Su, Sv$ is an orthonormal list in $W$; in particular, $\norm{Su} = 1$.
    Now let $x \in V$.
    If $x = 0$, then $\norm{Sx} = 0 = \norm{x}$.
    Otherwise, $\frac{x}{\norm{x}}$ has norm $1$.
    Then
    \[
        \norm{Sx} = \norm*{S\left(\frac{x}{\norm{x}}\right)} \norm{x} = \norm{x}.
    \]
    Hence, $S$ preserves norms so that $S$ is an isometry.
\end{soliff}
...
\begin{sol}
    ...
    For any $v \in V$, write $v = u + w$ with $u \in U$ and $w \in U^\perp$ by 6.49.
    By 6.57(b) and (c),
    \[
        (2P - I) v = (2P - I)(u + w) = 2P(u + w) - (u + w) = 2u - (u + w) = u - w = Su + Sw = Sv.
    \]
    Hence, $S = 2P - I$.
    By \exref{7a20}, $P$ is self-adjoint.
    It follows that $S^\ast = 2P^\ast - I = 2P - I = S$ by 7.5.
    By 6.57, $P^2 = P$, so
    \[
        S^2 = (2P - I)^2 = 4P^2 - 4P + I = 4P - 4P + I = I
    \]
    Therefore, $SS^\ast = S^\ast S = S^2 = I$, and $S$ is a self-adjoint unitary operator by 7.53(b).
\end{sol}
...
\begin{sol}
    Observe that every eigenvalue of $T$ is real by 7.12.
    Therefore, neither $i$ nor $-i$ is an eigenvalue of $T$ so that $T - iI$ and $T + iI$ are invertible.
    Define $S = (T + iI)(T - iI)\inv$.
    By 7.5, $(T + iI)^\ast = T^\ast - iI = T - iI$ and $((T - iI)\inv)^\ast = ((T - iI)^\ast)\inv = (T + iI)\inv$.
    Thus,
    \[
        S^\ast = (T + iI)\inv (T - iI).
    \]
    Because $T - iI$ and $T + iI$ commute,
    \[
        S^\ast S = (T + iI)\inv (T - iI) (T + iI)(T - iI)\inv = I,
    \]
    showing that $S$ is an isometry on $V$ by 7.49(b), whence $S$ is unitary.
...
\begin{sol}
    ...
    \begin{subexercises}
        ...
        \item Define $R \in \lin{\cbb^n}$ by $R(z_0, z_1, \ldots, z_{n-1}) = (z_{n-1}, z_{n-2}, \ldots, z_0)$.
        Then for any $z \in \cbb^n$ and $j \in \set{0, \ldots, n - 1}$,
        \[
            (\fcal^2 z)_j = \frac{1}{\sqrt n}\sum_{m=0}^{n-1} (\fcal z)_m e^{-2\pi i jm/n} = \frac 1n \sum_{m=0}^{n-1} \sum_{k=0}^{n-1} z_k e^{-2\pi i m(k+j)/n} = \sum_{k=0}^{n-1} z_k \left( \frac 1n \sum_{m=0}^{n-1} e^{-2\pi i m(k+j)/n} \right)
        \]
        where $r_k = e^{-2\pi i (k+j)/n}$.
        Observe that $0 \leq k+j \leq 2n-2$, so $r_k \neq 1$ unless $k+j = 0$ or $k+j = n$ in which case the inner sum evaluates to 1.
        Otherwise, we have $r_k \neq 1$ and $r_k^n = 1$, so
        \[
            \frac 1n \sum_{m=0}^{n-1} r_k^m = \frac 1n \cdot \frac{1 - r_k^n}{1-r_k} = 0.
        \]
        Observe that for $k \in \set{0, \ldots, n-1}$, one such $k$ exists for which $k+j = 0$ or $k+j = n$, namely $k = 0$ if $j = 0$ or $k = n-j$ if $1 \leq j \leq n-1$.
        Hence, $(\fcal^2 z)_0 = z_0$ and for $j \geq 1$, $(\fcal^2 z)_j = z_{n-j}$, whereupon $\fcal^2 z = Rz$.
        Since $R$ fixes the $0$th coordinate and reverses the $j$th and $(n-j)$th coordinates for $j \geq 1$, $R$ is its own inverse, i.e., $R^2 = I$.
        From this, we get $\fcal(\fcal R) = \fcal^2 R = R^2 = I$.
        By 3.68, $(\fcal R)\fcal = I$, and hence $\fcal\inv = \fcal R$.
        Because $z_n = z_0$,
        \[
            \fcal\inv(z_0, z_1, \ldots, z_{n-1}) = (\fcal R)(z_0, z_1, \ldots, z_{n-1}) = \fcal(z_0, z_{n-1}, \ldots, z_1).
        \]
        \item By part (b), $\fcal^2 = R$ and $R^2 = I$, so $\fcal^4 = R^2 = I$. \qedhere
    \end{subexercises}
\end{sol}
