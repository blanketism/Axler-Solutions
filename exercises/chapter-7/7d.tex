...
\begin{exercise}{7d2}
    ...
    \note{The phrase ``$T$ preserves orthogonality'' means that $\gen{Tu, Tv} = 0$ for all $u, v \in V$ such that $\gen{u, v} = 0$.}
\end{exercise}
...
\begin{exercise}{7d14}
    ...
    Prove that there exists a unitary operator $S \in \lin{V}$ such that $\gen{Sv, v} = b$ if and only if $|b| \leq 1$.
\end{exercise}
