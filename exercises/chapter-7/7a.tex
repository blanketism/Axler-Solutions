...
\begin{sol}
    ...
    \[
        \ip{T(z_1, \ldots, z_n), (w_1, \ldots, w_n)} = \ip{(0, z_1, \ldots, z_{n-1}), (w_1, \ldots, w_n)} = z_1\bar{w_2} + \cdots + z_{n-1}\bar{w_n}.
    \]
    We need
    \[
        \ip{(z_1, \ldots, z_n), T^\ast(w_1, \ldots, w_n)} = z_1\bar{w_2} + \cdots + z_{n-1}\bar{w_n}.
    \]
...
\begin{sol}
    ...
    \[
        \norm{Tv}^2 = \ip{Tv, Tv} = \ip{v, T^\ast Tv} = \ip{v, 0} = 0.
    \]
...
\begin{sol}
    \begin{align*}
        ...
        &\iff \ip{u, T^\ast v} = 0 \text{ for all } u \in U, v \in U^\perp \\
        &\iff \ip{Tu, v} = 0 \text{ for all } u \in U, v \in U^\perp \\
...
\begin{sol}
    Let
    \[
        Te_j = \sum_{k=1}^m \ip{Te_j, f_k} f_k \implies \norm{Te_j}^2 = \sum_{k=1}^m \abs{\ip{Te_j, f_k}}^2.
    \]
    Similarly, we may write
    \[
        T^\ast f_i = \sum_{\ell=1}^n \ip{T^\ast f_i, e_\ell} e_\ell \implies \norm{T^\ast f_i}^2 = \sum_{\ell=1}^n \abs{\ip{T^\ast f_i, e_\ell}}^2.
    \]
    Note that
    \[
        \ip{T^\ast f_i, e_j} = \ip{f_i, Te_j} = \bar{\ip{Te_j, f_i}} \implies \abs{\ip{T^\ast f_i, e_j}}^2 = \abs{\ip{Te_j, f_i}}^2.
    \]
    Therefore, we have
    \[
        \sum_{k=1}^m \norm{T^\ast f_k}^2 = \sum_{k=1}^m \sum_{\ell=1}^n \abs{\ip{Te_\ell, f_k}}^2 = \sum_{\ell=1}^n \sum_{k=1}^m \abs{\ip{Te_\ell, f_k}}^2 = \sum_{\ell=1}^n \norm{Te_\ell}^2. \qedhere
    \]
\end{sol}
...
\begin{exercise}{7a10}
    ...
    \[
        \ip{Tv, v} = \ip{T^\ast v, v}
    \]
    for all $v \in V$.
\end{exercise}

\begin{soliff}
    \rightimp Suppose $\ip{Tv, v} = \ip{T^\ast v, v}$ for all $v \in V$.
    Then for all $v \in V$,
    \[
        \ip{(T - T^\ast)v, v} = 0.
    \]
...
\begin{solenum}
    ...
    \[
        \ip{S(w, z), (x, y)} = \ip{(-z, w), (x, y)} = -z\bar x + w\bar y.
    \]
    Suppose $S^\ast(x, y) = (u, v)$. Then
    \[
        \ip{S(w, z), (x, y)} = \ip{(w, z), S^\ast(x, y)} = \ip{(w, z), (u, v)} = w\bar u + z\bar v.
    \]
...
\begin{solenum}
    ...
    If $\acal T = \lambda T$ for some nonzero $T \in \lin{V}$ and $\lambda \in \rbb$, then $T^\ast = \lambda T$.
    Taking the adjoint of both sides gives
    \[
        T = \lambda T^\ast = \lambda (\lambda T) = \lambda^2 T.
    \]
    Hence, $\lambda^2 = 1$, so $\lambda = 1$ or $\lambda = -1$.
    To see that $\lambda = 1$ always occurs as an eigenvalue for $\dim V \geq 1$, note that $I^\ast = I$, so $\acal I = I = 1 \cdot I$.
    If $\dim V = 1$, then $T = \mu I$ for some $\mu \in \rbb$.
    ...
    Define $S \in \lin{V}$ by $S e_1 = e_2$, $S e_2 = -e_1$, and $S e_j = 0$ for $j \geq 3$.
    Then $S^\ast e_1 = -e_2$, $S^\ast e_2 = e_1$, and $S^\ast e_j = 0$ for $j \geq 3$, so $S^\ast = -S$.
    Hence, $-1$ is also an eigenvalue of $\acal$.
    Therefore, the eigenvalues of $\acal$ are $1$ and $-1$ when $\dim V \geq 2$.
    ...
    If $V = \set 0$, then $\lin{V} = \set 0$, and the minimal polynomial of $\acal$ is $1$.
...
\begin{exercise}{7a14}
    Define an inner product on $\pcal_2(\rbb)$ by $\ip{p, q} = \int_0^1 pq$.
...
\begin{solenum}
    ...
    but
    \[
        \ip{p, Tq} = \ip{1, x} = \int_0^1 x \dd x = \frac{1}{2} \neq 0.
    \]
    Hence, $T$ is not self-adjoint.
    \item Observe that
    \[
        \norm{x}^2 = \ip{x, x} = \int_0^1 x^2 \dd x = \frac{1}{3}.
    \]
...
\begin{sol}
    We first note that
    \[
        TT\inv = I \implies (T\inv)^\ast T^\ast = I^\ast = I \implies (T\inv)^\ast = (T^\ast)\inv.
    \]
    \begin{subexercises}
        \item
        \begin{align*}
...
\begin{solenum}
    ...
    By 7.9, it follows that $T$ is self-adjoint if and only if $\mcal(T)$ is a symmetric matrix, i.e., $\mcal(T) = \mcal(T)\trans$.
...
\begin{sol}
    ...
    \[
        ...
        \end{pmatrix}.
    \]
    From this, we see that $\mcal(S) = \mcal(S)\trans$, so $S$ is normal.
    Moreover,
    \[
        \mcal(T) = 
        \begin{pmatrix}
...
\begin{sol}
    ...
    \[
        \norm{T^\ast v}^2 = \ip{T^\ast v, T^\ast v} = \ip{TT^\ast v, v}, \quad \norm{Tv}^2 = \ip{Tv, Tv} = \ip{T^\ast Tv, v}.
    \]
    From the hypothesis, we have $\norm{T^\ast v}^2 \leq \norm{Tv}^2$ for all $v \in V$, which gives
    \[
        \ip{(T^\ast T - TT^\ast)v, v} \geq 0 \quad \text{for all } v \in V.
    \]
    Let $S = T^\ast T - TT^\ast$.
    Then $S^\ast = (T^\ast T - TT^\ast)^\ast = T^\ast T - TT^\ast = S$, so $S$ is self-adjoint.
    Moreover, $\ip{Sv, v} \geq 0$ for all $v \in V$.
    Let $e_1, \ldots, e_n$ be an orthonormal basis of $V$.
    From \exref{7a5}, then
    \[
        \sum_{i=1}^n \norm{Te_i}^2 = \sum_{i=1}^n \ip{T^\ast Te_i, e_i} = \sum_{i=1}^n \ip{TT^\ast e_i, e_i} = \sum_{i=1}^n \norm{T^\ast e_i}^2.
    \]
    Then
    \[
        \sum_{i=1}^n \ip{Se_i, e_i} = \sum_{i=1}^n \ip{(T^\ast T - TT^\ast)e_i, e_i} = \sum_{i=1}^n \ip{T^\ast Te_i, e_i} - \sum_{i=1}^n \ip{TT^\ast e_i, e_i} = 0.
    \]
    Since each $\ip{Se_i, e_i} \geq 0$ and their sum is $0$, it follows that $\ip{Se_i, e_i} = 0$ for all $i$.
    For arbitrary $u \in V$ with $\norm u = 1$, we may extend it to an orthonormal basis $u, f_2, \ldots, f_n$ of $V$.
    Then
    \[
        \ip{Su, u} + \sum_{i=2}^n \ip{Sf_i, f_i} = \sum_{i=1}^n \ip{Se_i, e_i} = 0.
    \]
    Since each term is nonnegative, it follows that $\ip{Su, u} = 0$ for all unit vectors $u \in V$.
    By homogeneity, $\ip{Sv, v} = 0$ for all $v \in V$.
    From 7.16, if $S$ is self-adjoint and $\ip{Sv, v} = 0$ for all $v \in V$, then $S = 0$. 
...
\begin{sol}
    ...
    \[
        (\range P)^\perp = \nul P^\ast = \nul P.
    \]
    Since $P^2 = P$, $V = \nul P \oplus \range P$ is an orthogonal direct sum.
    Letting $U = \range P$, then any $v \in V$ can be written uniquely as $v = u + w$ with $u \in U$ and $w \in \nul P$.
    Hence,
    \[
    ...
    \[
        \ip{P_U v, w} = \ip{u_1, u_2 + w_2} = \ip{u_1, u_2} = \ip{u_1 + w_1, u_2} = \ip{v, P_U w}.
    \]
...
\begin{sol}
    ...
    Then
    \begin{align*}
        \norm{T(v + w)}^2 &= \ip{Tv + Tw, Tv + Tw} \\
        &= \ip{3v + 4w, 3v + 4w} \\
        &= \ip{3v, 3v} + \ip{3v, 4w} + \ip{4w, 3v} + \ip{4w, 4w} \\
        &= 9\ip{v, v} + 12\ip{v, w} + 12\ip{w, v} + 16\ip{w, w} \\
        &= 9\norm{v}^2 + 16\norm{w}^2 \\
        &= 9 \cdot 4 + 16 \cdot 4 \\
        &= 100.
    \end{align*}
...
\begin{sol}
    Note that
    \[
        \ip{Tv, w} = \ip{\ip{v, u}x, w} = \ip{v, u}\ip{x, w} = \ip{v, \bar{\ip{x, w}}u} = \ip{v, T^\ast w}.
    \]
    Since $\ip{v, u_1} = \ip{v, u_2}$ for all $v \in V$ implies $u_1 = u_2$, we conclude that $T^\ast w = \bar{\ip{x, w}}u = \ip{w, x}u$.
    \begin{subexercises}
        \item
        \rightimp If $T$ is self-adjoint, then $T = T^\ast$, which means $\ip{v, u}x = \ip{v, x}u$ for all $v \in V$.
        If $u = 0$ or $x = 0$, then the list $u, x$ is linearly dependent.
        If both are nonzero, then letting $v = u$ gives $\norm{u}^2 x = \ip{u, x}u$, hence
        \[
            x = \frac{\ip{u, x}}{\norm{u}^2} u.
        \]
        ...
        \[
            Tv = \ip{v, u}x = \ip{v, u} \lambda u = \lambda \ip{v, u} u = T^\ast v,
        \]
        hence $T$ is self-adjoint.
        \item
        \rightimp Suppose $T$ is normal. Then for any $y \in V$,
        \[
            TT^\ast y = T(\ip{y, x}u) = \ip{y, x}Tu = \ip{y, x}\ip{u, u}x = \norm{u}^2 \ip{y, x}x,
        \]
        and
        \[
            T^\ast T y = T^\ast(\ip{y, u}x) = \ip{y, u}T^\ast x = \ip{y, u}\ip{x, x}u = \norm{x}^2 \ip{y, u}u.
        \]
        If $u = 0$ or $x = 0$, then the list $u, x$ is linearly dependent.
        Otherwise, for nonzero $u$ and $x$, we must have $\norm{u}^2 \ip{y, x}x = \norm{x}^2 \ip{y, u}u$ for all $y \in V$.
        Let $y = u$. Then $\norm{u}^2 \ip{u, x}x = \norm{x}^2 \norm{u}^2 u$, hence $\ip{u, x}x = \norm{x}^2 u$.
        If $x \neq 0$, then
        \[
            u = \frac{\ip{u, x}}{\norm{x}^2} x,
        \]
        ...
        Then for all $y \in V$,
        \[
            TT^\ast y = \norm{u}^2 \ip{y, x}x = \norm{\mu x}^2 \ip{y, x}x = \abs{\mu}^2 \norm{x}^2 \ip{y, x}x,
        \]
        and
        \[
            T^\ast T y = \norm{x}^2 \ip{y, u}u = \norm{x}^2 \ip{y, \mu x} \mu x = \abs{\mu}^2 \norm{x}^2 \ip{y, x}x.
        \]
...
\begin{sol}
    ...
    Therefore, $T^\ast Tv = 0$ so that $\norm{Tv}^2 = \ip{Tv, Tv} = \ip{T^\ast Tv, v} = 0$.
...
\begin{sol}
    ...
    \[
        2\ip{(1, 1, 1), z} = \ip{T(1, 1, 1), z} = \ip{(1, 1, 1), T^\ast z} = \ip{(1, 1, 1), 0} = 0.
    \]
    Then $\ip{(1, 1, 1), z} = 0$.
    Direct calculation shows that
    \[
        \ip{(1, 1, 1), z} = \bar{z_1} + \bar{z_2} + \bar{z_3} = \bar{z_1 + z_2 + z_3}.
    \]
    Since $\ip{(1, 1, 1), z} = 0$, we have $\bar{z_1 + z_2 + z_3} = 0$, which implies $z_1 + z_2 + z_3 = 0$.
\end{sol}

\begin{exercise}{7a31}
    Fix a positive integer $n$.
    In the inner product space of continuous real-valued functions on $[-\pi, \pi]$ with inner product $\ip{f, g} = \int_{-\pi}^{\pi} fg$, let
    \[
...
\begin{solenum}
    ...
    \[
        \ip{Df, g} = \int_{-\pi}^{\pi} f'g = fg\,\big|_{-\pi}^{\pi} - \int_{-\pi}^{\pi} f g' = -\int_{-\pi}^{\pi} f g' = \ip{f, -Dg}.
    \]
...
\begin{sol}
    ...
    \[
        (T'(\phi_w))v = \phi_w(Tv) = \ip{Tv, w} = \ip{v, T^\ast w} = \phi_{T^\ast w}v.
    \]
