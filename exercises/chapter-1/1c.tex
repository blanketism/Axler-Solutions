...
\begin{exercise}{1c20}
    ...
    \]
    Find a subspace $W$ of $\fbb^4$ such that $\fbb^4 = U \oplus W$.
\end{exercise}
...
\begin{sol}
    ...
    Hence, every $x \in \fbb^4$ can be uniquely written as $x = u + w$ for some $u \in U$ and $w \in W$, so $\fbb^4 = U \oplus W$.
\end{sol}
...
\begin{exercise}{1c21}
    ...
    \]
    Find a subspace $W$ of $\fbb^5$ such that $\fbb^5 = U \oplus W$.
\end{exercise}
...
\begin{sol}
    ...
    Hence, every $x \in \fbb^5$ can be written as $x = u + w$ for some $u \in U$ and $w \in W$, so $\fbb^5 = U \oplus W$.
\end{sol}
...
\begin{exercise}{1c22}
    ...
    \[
    	U = \set{(x, y, x + y, x - y, 2x) \in \fbb^5 \mid x, y \in \fbb}.
    \]
    Find three subspaces $W_1, W_2, W_3$ of $\fbb^5$, none of which equals $\set 0$, such that $\fbb^5 = U \oplus W_1 \oplus W_2 \oplus W_3$.
\end{exercise}
...
\begin{sol}
    ...
    \]
    It is straightforward to show that $W = W_1 \oplus W_2 \oplus W_3$.
    By \exref{1c21}, we have that $\fbb^5 = U \oplus W$.
    By uniqueness of the decomposition of $v \in \fbb^5$ as $v = u + w$ for some $u \in U$ and $w \in W$, and uniqueness of the decomposition of $w \in W$ as $w = w_1 + w_2 + w_3$ for some $w_1 \in W_1$, $w_2 \in W_2$, and $w_3 \in W_3$, we have that $\fbb^5 = U \oplus W_1 \oplus W_2 \oplus W_3$.
\end{sol}
...
\begin{sol}
    ...
    Hence, $V = V_1 \oplus U$ and $V = V_2 \oplus U$, but $V_1 \neq V_2$.
\end{sol}
...
\begin{exercise}{1c24}
	...
	Show that $\rbb^{\rbb} = V_e \oplus V_o$.
\end{exercise}

\begin{sol}
    Let $f(x) = 0$ for all $x \in \rbb$.
    Since $f(-x) = f(x) = 0$, and $f(-x) = -f(x) = 0$, we have $f(x) \in V_e$ and $f(x) \in V_o$.
    Now suppose $f_1, f_2 \in V_e$ and $\alpha \in \rbb$.
    Then
    \[
        (f_1 + f_2)(-x) = f_1(-x) + f_2(-x) = f_1(x) + f_2(x) = (f_1 + f_2)(x)
    \]
    Hence, $f_1 + f_2 \in V_e$.
    Also,
    \[
        (\alpha f_1)(-x) = \alpha f_1(-x) = \alpha f_1(x) = (\alpha f_1)(x),
    \]
    so $\alpha f_1 \in V_e$.
    Therefore, $V_e$ is a subspace of $\rbb^{\rbb}$.

    Similarly, if $g_1, g_2 \in V_o$ and $\beta \in \rbb$, then
    \[
        (g_1 + g_2)(-x) = g_1(-x) + g_2(-x) = -g_1(x) - g_2(x) = -(g_1 + g_2)(x)
    \]
    Hence, $g_1 + g_2 \in V_o$.
    Also,
    \[
        (\beta g_1)(-x) = \beta g_1(-x) = \beta (-g_1(x)) = -(\beta g_1)(x),
    \]
    so $\beta g_1 \in V_o$.
    Therefore, $V_o$ is a subspace of $\rbb^{\rbb}$.

    We now show $\rbb^{\rbb} = V_e + V_o$.
    Let $g \in \rbb^{\rbb}$.
    Define the functions $g_e$ and $g_o$ by
    \[
        g_e(x) = \frac{g(x) + g(-x)}{2}, \quad g_o(x) = \frac{g(x) - g(-x)}{2}.
    \]
    Then
    \[
        g_e(-x) = \frac{g(-x) + g(x)}{2} = \frac{g(x) + g(-x)}{2} = g_e(x),
    \]
    and
    \[
        g_o(-x) = \frac{g(-x) - g(x)}{2} = -\frac{g(x) - g(-x)}{2} = -g_o(x).
    \]
    Hence, $g_e \in V_e$ and $g_o \in V_o$.
    Moreover,
    \[
        g(x) = g_e(x) + g_o(x),
    \]
    ...
    Finally, if $h \in V_e \cap V_o$, then $h(-x) = h(x)$ and $h(-x) = -h(x)$ for all $x \in \rbb$.
    Hence, $h(x) = -h(x)$, so $h(x) = 0$ for all $x \in \rbb$.
    Therefore, $V_e \cap V_o = \set 0$, and we conclude that $\rbb^{\rbb} = V_e \oplus V_o$.
\end{sol}