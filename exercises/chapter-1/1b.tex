...
\begin{exercise}[1b6]
	...
	Define an addition and scalar multiplication on $\r \cup \set{\infty, -\infty}$ as you could guess from the notation.
	Specifically, the sum and product of two real numbers is as usual, and for $t \in \r$, define
    \[t\infty = 
    \begin{cases}
        -\infty & \text{if } t < 0, \\
        0 & \text{if } t = 0, \\
    ...
    \end{align*}
    With these operations of addition and scalar multiplication, is $\r \cup \set{\infty, -\infty}$ a vector space over $\r$?
	Explain.
\end{exercise}

\begin{sol}
    No, $\r \cup \set{\infty, -\infty}$ is not a vector space over $\r$ as it does not satisfy the associative property of addition.
	Let $t \in \r \setminus \set 0$.
	Then
...
\begin{sol}
	...
	Then for all $s \in S$,
        ...
        \item \textbf{Associativity}: Let $f, g, h \in V^S$ and $t, u \in \f$.
	Then for all $s \in S$,
        \begin{align*}
            ((f + g) + h)(s) &= (f + g)(s) + h(s) \\
            &= (f(s) + g(s)) + h(s) \\
	...
	Then for all $s \in S$,
        ...
        \[(1f)(s) = 1(f(s)) = f(s).\]
        \item \textbf{Distributive properties}: Let $f, g \in V^S$ and $t, u \in \f$.
	Then for all $s \in S$,
        \begin{align*}
            ((t + u)f)(s) & = (t + u)(f(s)) \\
            & = t(f(s)) + u(f(s)) \\
...
\begin{exercise}[1b8]
    ...
    \begin{itemize}
        \item The \textbf{complexification} of $V$, denoted by $V_{\c}$, equals $V \times V$.
	An element of $V_{\c}$ is an ordered pair $(u, v)$, where $u, v \in V$, but we write this as $u + iv$.
...
\begin{sol}
    ...
    \begin{enumerate}
        \item \textbf{Commutativity}: Let $u_1 + v_1i, u_2 + v_2i \in V_{\c}$.
	Then
        \[(u_1 + v_1i) + (u_2 + v_2i) = (u_1 + u_2) + i(v_1 + v_2) = (u_2 + u_1) + i(v_2 + v_1) = (u_2 + v_2i) + (u_1 + v_1i).\]
        \item \textbf{Associativity}: Let $u_1 + v_1i, u_2 + v_2i, u_3 + v_3i \in V_{\c}$.
	Then
        \begin{align*}
        ...
        \end{align*}
        Similarly, let $a + bi, c + di \in \c$ and $u + vi \in V_{\c}$.
	Then
        \begin{align*}
	...
	Then for all $u + vi \in V_{\c}$,
        ...
        \item \textbf{Additive inverse}: Let $u + vi \in V_{\c}$.
	Then
        ...
        \item \textbf{Distributive properties}: Let $a + bi, c + di \in \c$ and $u_1 + v_1i, u_2 + v_2i \in V_{\c}$.
	Then
        \begin{align*}
