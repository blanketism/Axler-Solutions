...
\begin{sol}
    ...
    \begin{subexercises}
        ...
        \item $|wz| = |(c + di)(a + bi)| = |(ac - bd) + (ad + bc)i| = \sqrt{(ac - bd)^2 + (ad + bc)^2} = \sqrt{(a^2 + b^2)(c^2 + d^2)} = |a + bi| |c + di| = |z| |w|$. \qedhere
    \end{subexercises}
\end{sol}
...
\begin{sol}
    ...
    Therefore,
    \[
    	\sigma(v) - i\sigma(iv) = s - i(-t) = s + ti = \phi(v). \qedhere
    \]
\end{sol}
...
\begin{sol}
	...
	Since $p$ has degree $m$, it can have at most $m$ zeros by the fundamental theorem of algebra.
    Therefore, $p$ must have exactly $m$ distinct zeros.
\end{sol}
...
\begin{sol}
	...
	Since non-real zeros come in pairs, there are an even number of non-real zeros.
...
\begin{sol}
	...
	By \exref{4-7} with $\fbb = \rbb$, there exists a polynomial $q \in \pcal_m(\rbb)$ that satisfies $q(x_k) = y_k$ for each $k = 0, 1, \ldots, m$.
...
\begin{solenum}
    ...
    \]
	Therefore, $q = 0$, hence $s = 0$, and $T$ is an isomorphism.
    We conclude that $\dim \pcal(\fbb) / U = \dim \pcal_{\deg p - 1}(\fbb) = \deg p$.
...
\begin{solenum}
    ...
    Then $rp = -sq$ implies $rp$ is a polynomial multiple of $q$, so every zero of $q$ must be a zero of $rp$.
    Because $p$ and $q$ have no zeros in common, every zero of $q$ must be a zero of $r$. But $\deg r < \deg q$, so it must be that $r = 0$.
    Because $r = 0$, we have $-sq = 0$. Then since $q$ is nonconstant, it must be that $s = 0$.
    Therefore, if $(r,s) \in \nul T$, we have $(r,s) = 0$, so $T$ is injective.
