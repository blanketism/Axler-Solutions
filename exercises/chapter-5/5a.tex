...
\begin{sol}
	...
	If $u$ and $w$ were linearly dependent, let $u = \alpha w$ for some scalar $\alpha \in \fbb$.
...
\begin{sol}
	...
	Now let $S$ be the set in question, and let $p, q \in \pcal(\fbb)$ where
	...
	By commutativity of $\fbb$, we see that $q(T)p(T) = p(T)q(T)$, so elements of $S$ commute.
