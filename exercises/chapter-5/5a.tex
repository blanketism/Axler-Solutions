...
\begin{sol}
	...
	Therefore, the eigenvectors corresponding to the eigenvalue $\lambda = 0$ are all nonzero constant polynomials. \qedhere
    \end{itemize}
\end{sol}
...
\begin{sol}
    ...
    \[
    	|\lambda| \leq n \max\{|\mcal(T)_{j, k}| : 1 \leq j, k \leq n\}. \qedhere
    \]
\end{sol}
...
\begin{solenum}
    \item Suppose the sum of the entries in each row of $A$ equals $1$.
	Let $v = (1, 1, \ldots, 1)\trans \in \fbb^n$.
	Then
    \[
    	Tv = Av = v,
    \]
    ...
    \item Suppose the sum of the entries in each column of $A$ equals $1$.
	By part (a), this is equivalent to saying that the sum of the entries in each row of $A\trans$ equals $1$, hence $1$ is an eigenvalue of the linear operator defined by $A\trans$.
	But the eigenvalues of $A\trans$ are the same as the eigenvalues of $A$ because $A - \lambda I$ is not invertible if and only if $A\trans - \lambda I$ is not invertible.
...
\begin{exercise}{5a27}
    ...
    \apnote{5a27}{ for a more elegant proof that doesn't require construction of subspaces.}
\end{exercise}
...
\begin{sol}
    ...
    \]
    Since $T$ has no eigenvalues, $T - I$ and $T + I$ are both injective, hence $T^2 - I$ is injective.
    Then for any $v \in V$, we have
    \[
    	(T^2 - I)(T^2 + I)v = 0,
    \]
    which implies $(T^2 + I)v = 0$ since $T^2 - I$ is injective.
    Therefore, $T^2 + I = 0$, or $T^2 = -I$.
\end{sol}
...
\begin{solenum}
    ...
    \end{align*}
    \item Let $\lambda$ be an eigenvalue of $T/U$ with eigenvector $v+U \neq 0$.
    Then $v \notin U$ and $(T-\lambda I)v \in U$.
    Consider $(T-\lambda I) \vert_U \in \mathcal{L}(U)$.
    \begin{itemize}
        \item If $(T-\lambda I) \vert_U$ is not injective, then $\lambda$ is an eigenvalue of $T \vert_U$ and hence of $T$.
        \item If $(T-\lambda I) \vert_U$ is injective, then by finite-dimensionality it is surjective onto $U$. Thus, there exists $u \in U$ with $(T-\lambda I)u = (T-\lambda I)v$.
        Consequently, $(T-\lambda I)(v-u)=0$ with $v-u \neq 0$ (since $v \notin U$ and $u \in U$), showing $\lambda$ is an eigenvalue of $T$.
    \end{itemize}
    In either case, $\lambda$ is an eigenvalue of $T$.
\end{solenum}
...
\begin{exercise}{5a43}
    ...
    \apnote{5a43}{ for a proof that utilizes commutativity of operators in $\set{p(T) \mid p \in \pcal(\fbb)}$.}
\end{exercise}

\begin{sol}
    If $T$ is the zero transformation or a scalar multiple of the identity, the result trivially holds. 
    Otherwise, because $\dim V > 1$, there is at least one nonzero vector $v_1 \in V$ that is not an eigenvector of $T$.
    Let $Tv_1 = v_2$, so that $v_1$ and $v_2$ are linearly independent, and extend $v_1, v_2$ to a basis $v_1, \dots, v_n$ for $V$.

    Now suppose that $\set{p(T) \mid p \in \pcal(\fbb)} = \lin{V}$ and consider the map $S \in \lin{V}$ defined by $Sv_1 = 0$ and $Sv_i = v_i$ for $i = 2,\dots,n$. Because $S \in \lin{V}$, there must be a polynomial $p \in \pcal(\fbb)$ such that $p(T) = S$.
    Because the null space of $p(T)$ is invariant under $T$, it must be that the null space of $S$ is invariant under $T$. 
    This is impossible, though, as $Sv_1 = 0$ and $STv_1 = Sv_2 = v_2 \neq 0$. 
    Therefore, there is no $T \in \lin{V}$ such that $\set{p(T) \mid p \in \pcal(\fbb)} = \lin{V}$.
\end{sol}