...
\begin{sol}
    ...
    From $Tv_j = \lambda_j v_j$ for $j = 2, \ldots, m$, then $T({\lambda_j}\inv v_j) = v_j$ implies that $v_j \in \range T$ for $j = 2, \ldots, m$.
...
\begin{sol}
    ...
    Since $p(T)q(T) = q(T)p(T)$, it follows that $S$ is a commutative set of operators.
