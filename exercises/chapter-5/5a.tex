...
\begin{exercise}[5a5]
    Suppose $T \in \lin{\r^2}$ is defined by $T(x, y) = (-3y, x)$.
...
\begin{exercise}[5a6]
    Define $T \in \lin{\f^2}$ by $T(w, z) = (z, w)$.
...
\begin{exercise}[5a7]
    Define $T \in \lin{\f^3}$ by $T(z_1, z_2, z_3) = (2z_2, 0, 5z_3)$.
	Find all eigenvalues and eigenvectors of $T$.
\end{exercise}
...
\begin{sol}
    ...
    \begin{itemize}
        \item If $\lambda \neq 0$, then $\deg p' = \deg p$, which is a contradiction since $\deg p' = \deg p - 1$.
	Therefore, there are no eigenvectors corresponding to nonzero eigenvalues.
...
\begin{exercise}[5a10]
    Define $T \in \lin{\pp_4(\r)}$ by $(Tp)(x) = xp'(x)$ for all $x \in \r$.
	Find all eigenvalues and eigenvectors of $T$.
\end{exercise}
...
\begin{sol}
    ...
    Equating coefficients with $\lambda p(x) = \lambda a_0 + \lambda a_1x + \lambda a_2x^2 + \lambda a_3x^3 + \lambda a_4x^4$, we get that $T(x^n) = nx^n$ for $n = 0, 1, 2, 3, 4$.
	Therefore, the eigenvalues of $T$ are $\lambda = 0, 1, 2, 3, 4$.
	Since $\dim \pp_4(\r) = 5$, and we have found 5 distinct eigenvalues, the corresponding eigenvectors are the nonzero scalar multiples of $1, x, x^2, x^3, x^4$.
\end{sol}

\begin{exercise}[5a11]
    Suppose $V$ is finite-dimensional, $T \in \lin{V}$, and $\alpha \in \f$.
	Prove that there exists $\delta > 0$ such that $T - \lambda I$ is invertible for all $\lambda \in \f$ such that $0 < |\alpha - \lambda| < \delta$.
\end{exercise}

\begin{sol}
    Since $V$ is finite-dimensional, the set of eigenvalues of $T$ is finite.
	If $T$ has no eigenvalues, then $T - \lambda I$ is invertible for all $\lambda \in \f$, and we can choose any $\delta > 0$.
	...
	Otherwise, we have for any $\lambda \in \f$ such that $0 < |\alpha - \lambda| < \delta$, we have $\lambda \neq \lambda_j$ for all $j = 1, \ldots, m$, so $\lambda$ is not an eigenvalue of $T$.
...
\begin{exercise}[5a15]
    Suppose $V$ is finite-dimensional, $T \in \lin{V}$, and $\lambda \in \f$.
...
\begin{sol}
	...
	Let $\mm(T)$ be the matrix of $T$ with respect to the basis $v_1, \ldots, v_n$.
	Then
...
\begin{exercise}[5a17]
    Suppose $\f = \r$, $T \in \lin{V}$, and $\lambda \in \r$.
	Prove that $\lambda$ is an eigenvalue of $T$ if and only if $\lambda$ is an eigenvalue of the complexification $T_\c$.
...
\begin{sol}
	...
	Then in the complexification, we have $T_\c(v + i0) = Tv + iT0 = \lambda v + i0 = \lambda (v + i0)$, showing that $\lambda$ is an eigenvalue of $T_\c$ with eigenvector $v + i0$.

    Conversely, suppose $\lambda$ is an eigenvalue of $T_\c$ with eigenvector $v + iw \in V_\c$, where $v, w \in V$.
	Then
...
\begin{exercise}[5a18]
	...
	Prove that $\lambda$ is an eigenvalue of the complexification $T_\c$ if and only if $\bar{\lambda}$ is an eigenvalue of $T_\c$.
\end{exercise}

\begin{sol}
    Suppose $\lambda$ is an eigenvalue of $T_\c$ with eigenvector $v + iw \in V_\c$, where $v, w \in V$.
	Then
    \[T_\c(v + iw) = Tv + iTw = \lambda (v + iw) = \lambda v + i\lambda w.\]
    Moreover, we have
    \[T_\c(v - iw) = Tv - iTw.\]
    ...
    Conversely, if $\bar{\lambda}$ is an eigenvalue of $T_\c$, then by the same argument, $\lambda$ is an eigenvalue of $T_\c$.
...
\begin{sol}
    Suppose $\lambda \in \f$ is an eigenvalue of $T$ with eigenvector $(z_1, z_2, \ldots) \in \f^\infty$.
	Then
...
\begin{sole}
    \item Let $\alpha \in \f$.
	Consider the vector $(1, \alpha, \alpha^2, \ldots) \in \f^\infty$.
	Then
	...
	Since $\alpha \in \f$ was arbitrary, every element of $\f$ is an eigenvalue of $S$.
    \item Let $\alpha \in \f$ and consider the eigenvalue $\alpha$ of $S$.
...
\begin{sol}
    ...
    If $\lambda w = 0$, then either $\lambda = 0$ or $w = 0$.
	If $w = 0$, then $Tv = 0$, which implies $STv = S(Tv) = S0 = 0$.
	But $STv = \lambda v$, so $\lambda v = 0$.
	Since $v \neq 0$, we must have $\lambda = 0$.
	Then this implies $ST$ is not invertible so that $TS$ is also not invertible, meaning $0$ is an eigenvalue of $TS$.
	Therefore, every eigenvalue of $ST$ is an eigenvalue of $TS$.
	By symmetry, the converse also holds, so $ST$ and $TS$ have the same eigenvalues.
\end{sol}

\begin{exercise}[5a24]
    Suppose $A$ is an $n$-by-$n$ matrix with entries in $\f$.
	Define $T \in \lin{\f^n}$ by $Tx = Ax$, where elements of $\f^n$ are thought of as $n$-by-$1$ column vectors.
    \begin{subexercises}
...
\begin{sol}
    Let $Tu = \lambda u$ and $Tw = \mu w$ for some scalars $\lambda, \mu \in \f$.
	Since $u + w$ is also an eigenvector of $T$, we have $T(u + w) = \nu (u + w)$ for some scalar $\nu \in \f$.
	Then
    \[\lambda u + \mu w = Tu + Tw = T(u + w) = \nu (u + w) = \nu u + \nu w.\]
    Since $u$ and $w$ are eigenvectors, they are nonzero.
	If $u$ and $w$ were linearly dependent, let $u = \alpha w$ for some scalar $\alpha \in \f$.
	Then $Tu = \lambda u = \lambda \alpha w$, and $Tu = T(\alpha w) = \alpha Tw = \alpha \mu w$.
	Equating these, we get $\lambda \alpha w = \alpha \mu w$, and since $\alpha \neq 0$ and $w \neq 0$, it follows that $\lambda = \mu$.
	Hence, $u$ and $w$ correspond to the same eigenvalue.
    
    If $u$ and $w$ are linearly independent, we can equate coefficients of $u$ and $w$ in \(\lambda u + \mu w = \nu u + \nu w\) to get $\lambda = \nu = \mu$.
	Therefore, in either case, $u$ and $w$ are eigenvectors corresponding to the same eigenvalue.
\end{sol}
...
\begin{sol}
	...
	By assumption, $v$ is an eigenvector of $T$, so $Tv = \lambda_v v$ for some scalar $\lambda_v \in \f$.
	Now, let $u \in V$ be another nonzero vector.
	Consider the vector $u + v$.
	If $u$ and $v$ are linearly dependent, then without loss of generality, let $u = \alpha v$ for some scalar $\alpha \in \f$.
    ...
    The remaining case is when $u + v$ is nonzero such that they are linearly independent.
	In this case, $u + v$ is an eigenvector of $T$, so $T(u + v) = \lambda_{u+v} (u + v)$ for some scalar $\lambda_{u+v} \in \f$.
    ...
    Since $u$ and $v$ were arbitrary nonzero vectors, it follows that there exists a scalar $\lambda \in \f$ such that $Tv = \lambda v$ for all nonzero $v \in V$.
	Hence, $T = \lambda I$.
\end{sol}
...
\begin{sol}
	...
	Then $v_2, \ldots, v_m$ are $m - 1$ eigenvectors corresponding to nonzero eigenvalues.
	Since $Tv_i = \lambda_i v_i \neq 0$ for $i = 2, \ldots, m$, it follows that $v_i \in \range T$.
	Therefore, $v_2, \ldots, v_m$ are linearly independent vectors in $\range T$, which implies that $m - 1 \leq \dim \range T$.
	Hence, the total number of distinct eigenvalues is at most $1 + \dim \range T$.
\end{sol}

\begin{exercise}[5a29]
    Suppose $T \in \lin{\r^3}$ and $-4, 5$, and $\sqrt{7}$ are eigenvalues of $T$.
	Prove that there exists $x \in \r^3$ such that $Tx - 9x = (-4, 5, \sqrt{7})$.
\end{exercise}

\begin{sol}
    Since $\dim \r^3 = 3$, the eigenvalues of $T$ are exactly $-4, 5$, and $\sqrt 7$.
	It follows that 9 is not an eigenvalue of $T$, and $\nul(T - 9I) = \set 0$.
	Then $T - 9I$ is injective, and since $\r^3$ is finite-dimensional, $T - 9I$ is also surjective.
	Therefore, for any vector in $\r^3$, there exists $x \in \r^3$ such that $(T - 9I)x = (-4, 5, \sqrt{7})$.
\end{sol}
...
\begin{exercise}[5a35]
	...
	Prove that the list $e^{\lambda_1 x}, \ldots, e^{\lambda_n x}$ is linearly independent in the vector space of real-valued functions on $\r$.
...
\begin{exercise}[5a36]
	...
	Prove that the list $\cos(\lambda_1 x), \ldots, \\ \cos(\lambda_n x)$ is linearly independent in the vector space of real-valued functions on $\r$.
\end{exercise}
...
\begin{sol}
	...
	Consider a nonzero $\varphi \in V'$ and consider $S \in \lin{V}$ defined by $Sw = \varphi(w)v$ for all $w \in V$.
...
\begin{sole}
	...
	Linearity follows from the linearity of $T$: for any $\alpha, \beta \in \f$ and $v + U, w + U \in V / U$,
    \begin{align*}
	...
	This means $Tv - \lambda v \in U$.
	Suppose now that $T - \lambda I$ has no eigenvector in $V$ corresponding to $\lambda$.
	Since $U$ is invariant under $T - \lambda I$, then its restriction $(T - \lambda I)|_U$ is an operator on $U$.
	By the fundamental theorem of linear maps, we have
    \[\dim U = \dim \nul(T - \lambda I)|_U + \dim \range(T - \lambda I)|_U.\]
    Now for any $v + U \neq U$ with $(T - \lambda I)v \in U$, then surjectivity of $(T - \lambda I)|_U$ would imply that there exists $u \in U$ such that $(T - \lambda I)u = (T - \lambda I)v$.
	By injectivity, we get that $u = v$, which is a contradiction since $v \notin U$.
	Therefore, our assumption that $T - \lambda I$ has no eigenvector corresponding to $\lambda$ must be false, and $\lambda$ is indeed an eigenvalue of $T$.
\end{sole}
...
\begin{sol}
	...
	Extend to a subspace $U$ of $V$ of dimension $\dim V - 1$ that contains $\range(T - \lambda I)$.
	...
	By \autoref{ex:5a38} (b), any eigenvalue of $T / U$ is also an eigenvalue of $T$.
	Hence, $T$ has an eigenvalue.
\end{sol}

\begin{exercise}[5a40]
    Suppose $S, T \in \lin{V}$ and $S$ is invertible.
	Suppose $p \in \pp(\f)$ is a polynomial.
	Prove that
...
\begin{exercise}[5a41]
	...
	Prove that $U$ is invariant under $p(T)$ for every polynomial $p \in \pp(\f)$.
\end{exercise}
...
\begin{sol}
	...
	Therefore, $U$ is invariant under $p(T)$ for every polynomial $p \in \pp(\f)$.
\end{sol}
...
\begin{sole}
    \item Let $\lambda$ be an eigenvalue of $T$ with corresponding eigenvector $v = (x_1, x_2, \ldots, x_n) \in \f^n$.
    ...
    \item Let $U$ be an invariant subspace of $\f^n$ under $T$.
	Let
    ...
    The $i$-th term in this sum is $\alpha_i (i - i) e_i = 0$, and $u_1$ now has all terms except the $i$-th term.
...
\begin{exercise}[5a43]
	...
	Prove that $\set{p(T) \mid p \in \pp(\f)} \neq \lin{V}$.
\end{exercise}

\begin{sol}
    Note that $\dim \lin V \geq 4$.
	...
	Now let $S$ be the set in question, and let $p, q \in \pp(\f)$ where
    \[p(T) = a_0 I + a_1 T + \dots + a_m T^m, \quad q(T) = b_0 I + b_1 T + \dots + b_n T^n.\]
    Observe that $p(T)q(T)$ consists of linear combinations of powers of $T$ with coefficients $a_i b_j$.
	By commutativity of $\f$, we see that $q(T)p(T) = p(T)q(T)$, so elements of $S$ commute.
	Since $T_1$ and $T_2$ do not commute, they cannot both be in $S$.
	Hence, $S \neq \lin V$.
\end{sol}