...
\begin{sol}
    ...
    Consider the linear operator $T \in \lin{\rbb^2}$ defined by
    \[
        \begin{pmatrix}
            0 & 1 \\
            -1 & 0
        \end{pmatrix}
    \]
    which represents a $90^\circ$ rotation in the plane. 
...
\begin{sole}
    ...
    \]
    for the $i$-th diagonal entry. Since $A$ and $B$ are upper-triangular, then $k > i$ implies $a_{ik} = 0$ and $k < i$ implies $b_{ki} = 0$. Therefore, the only non-zero term in the sum occurs when $k = i$, giving us
    \[
...
\begin{sol}
    ...
    \[
        ...
        \end{pmatrix}.
    \]
    The matrix of $T$ with respect to the standard basis has only $0$'s on the diagonal, but $T$ is invertible with inverse
    \[
        T\inv = 
        \begin{pmatrix}
            0 & -1 \\
...
\begin{sole}
    \item Consider the list $v, Tv, T^2v, \ldots, T^{\dim V}v$.
